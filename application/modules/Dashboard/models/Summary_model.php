...
class Summary_model extends CI_Model {
	...
	public function get_patient_scaleup($filters){
		...
		$this->db->select("CONCAT_WS('/', data_month, data_year) period, SUM(IF(age_category = 'paed', total, NULL)) paed_total, SUM(IF(age_category = 'adult', total, NULL)) adult_total, ROUND(RAND()*150000)+650000  forecast", FALSE);
		if(!empty($filters)){
	...
	public function get_patient_services($filters){
		...
		$this->db->select("UPPER(county) county, SUM(IF(regimen_service= 'ART', total, 0)) art, SUM(IF(regimen_service= 'PMTCT', total, 0)) pmtct, SUM(IF(regimen_service= 'OI Only', total, 0)) oi, SUM(IF(regimen_service= 'HepB', total, 0)) hepb, SUM(IF(regimen_service= 'PrEP', total, 0)) prep, SUM(IF(regimen_service= 'PEP', total, 0)) pep", FALSE);
		if(!empty($filters)){
			foreach ($filters as $category => $filter) {
				$this->db->where_in($category, $filter);
			}
