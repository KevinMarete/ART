...
class Dashboard_model extends CI_Model {
	...
	public function get_facility_patient_distribution_numbers($filters){
		...
		$this->db->select("CONCAT(UCASE(SUBSTRING(facility, 1, 1)),LOWER(SUBSTRING(facility, 2))) name, FORMAT(SUM(IF(regimen_service = 'ART', total, NULL)), 0) ART, FORMAT(SUM(IF(regimen_service = 'PrEP', total, NULL)), 0) PREP, FORMAT(SUM(IF(regimen_service = 'PEP', total, NULL)), 0) PEP, FORMAT(SUM(CASE WHEN regimen_service = 'PMTCT' AND age_category = 'adult' THEN total ELSE 0 END), 0) 'PMTCT MOTHER', FORMAT(SUM(CASE WHEN regimen_service = 'PMTCT' AND age_category = 'paed' THEN total ELSE 0 END), 0) 'PMTCT CHILD', county as COUNTY, sub_county as 'SUB COUNTY'", FALSE);
		if(!empty($filters)){
			foreach ($filters as $category => $filter) {
				$this->db->where_in($category, $filter);
			}
