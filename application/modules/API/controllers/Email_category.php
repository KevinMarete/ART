 ...
 */
class Email_category extends \API\Libraries\REST_Controller  {

    function __construct()
    {
        parent::__construct();
        $this->load->model('email_category_model');
    }

    public function index_get()
    {
        // formulations from a data store e.g. database
        $formulations = $this->email_category_model->read();
    ...
    {   
        ...
        $data = $this->email_category_model->insert($data);
        if($data['status'])
        {
            unset($data['status']);
    ...
    {   
        ...
        $data = $this->email_category_model->update($id, $data);
        if($data['status'])
        {
            unset($data['status']);
            $this->set_response($data, \API\Libraries\REST_Controller::HTTP_CREATED); // CREATED (201) being the HTTP response code
    ...
    public function index_delete()
    {
        ...
        $data = $this->email_category_model->delete($id);
        if($data['status'])
        {
            unset($data['status']);
            $this->set_response([
